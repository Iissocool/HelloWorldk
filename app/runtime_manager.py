...
from .config import DATA_ROOT, MODELS_ROOT, SCRIPTS_ROOT, VENV_ROOT
...
RUNTIME_COMPONENTS: list[RuntimeComponentSpec] = [
    ...
    RuntimeComponentSpec(
        ...
        description="Windows 下的 AMD / Intel 通用 GPU 路线。",
        python_paths=[VENV_ROOT / "rembg-dml" / "Scripts" / "python.exe"],
    ),
    RuntimeComponentSpec(
        id="openvino",
        title="OpenVINO 运行时",
        description="Intel 设备的专项推理路线。",
        python_paths=[
...
def runtime_component_installed(component_id: str) -> bool:
    ...
    return any(path.exists() for path in component.python_paths)


def runtime_component_location(component_id: str) -> str:
    component = RUNTIME_COMPONENT_MAP[component_id]
    for path in component.python_paths:
        if path.exists():
